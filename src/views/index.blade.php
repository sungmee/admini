...
<div class="my-3 p-3 bg-white rounded shadow-sm">
    ...
    <div class="media text-muted pt-3" id="item-{{ $item->id }}">
        ...
        <p class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
            <strong class="d-block text-gray-dark"><a href="{{ route('page', ['page' => $item->slug]) }}" title="{{ trans('admini::post.index.view') }}" target="_blank">{{ $item->cn->title }}</a></strong>
            ...
            @if ($type == 'news')
            | <a class="del" href="javascript:;" data-id="{{ $item->id }}" data-title="{{ $item->cn->title }}" data-url="{{ route('admini.posts.destory', ['type' => $type, 'post' => $item->id]) }}">{{ trans('admini::post.index.destory') }}</a>
            @endif
        </p>
    </div>
    @empty
